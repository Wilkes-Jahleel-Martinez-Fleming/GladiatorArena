...
func actionTimer(glad Gladiator, commandChan chan<- Command) {
	for glad.health > 0 {
		wait := time.Duration(5000-glad.speed*250) * time.Millisecond
		time.Sleep(wait)

		fmt.Printf("\n[%s READY] Enter move (1: attack, 2: use potion): ", glad.name)
		var input int
		fmt.Scan(&input)
...
func applyMove(cmd Command, p1, p2 *Gladiator) {
	...
	case "defend":
		...
		attacker.health += 50
			fmt.Printf("\n %s uses a potion! (+50 health)\n", attacker.name)
		attacker.potions -= 1
		}else{
			fmt.Printf("\n %s reaches for a potion but nothing is there!", attacker.name)
		}
	default:
