...
func handleKeyPress(w http.ResponseWriter, r *http.Request) {
    ...
    // Prepare response with nicknames
    ...
    // Only include winner information if game is over
    if lobby.GameOver {
        responseData["winner"] = lobby.Winner
        responseData["winner_name"] = lobby.Players[lobby.Winner-1].Nickname
    }

    lobby.RoundResults = responseData
